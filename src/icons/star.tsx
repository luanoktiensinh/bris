...
export const StartIcon = ({ percent = 0, color }: { percent?: number, color?: string }) => {
  const { generateUniqueId, numBetween } = useHelpers();
  const id = useMemo(() => generateUniqueId(), [generateUniqueId]);
  return (
      <svg
          focusable="false"
      ...
      >
        <path
            ...
            fill={percent > 0 ? `url(#${id})` : 'currentColor'}
        ></path>
        {
          percent > 0 && (
              <defs>
                <linearGradient
                    id={id}
                    x1={`${numBetween(percent * 100, 0, 99.99)}%`}
                    y1="0%"
                    x2="100%"
                    y2="0%"
                >
                  <stop
                      offset="0%"
                      stopOpacity={1}
                      stopColor={color ?? 'var(--color-star)'}
                  ></stop>
                  <stop
                      offset="1%"
                      stopOpacity={1}
